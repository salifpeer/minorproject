...
<div id="cover">
  ...
  <li class="nav-item" role="presentation">
   ...
   <li class="nav-item" role="presentation">
    <a class="nav-link" id="pills-profile-tab" data-toggle="pill" href="#pills-addmission" role="tab" aria-controls="pills-profile" aria-selected="false">Addmission</a>
  </li>
...
?>
  ...
  <div class="tab-pane fade show  internal" id="pills-addmission" role="tabpanel" aria-labelledby="pills-home-tab">

    <div class="prescription"><br><br>
       
     <h1 style="color: #ee6e73;" class="text-center view"><i  class="fa fa-wheelchair" aria-hidden="true"></i>&nbspAdd<span style="color: white;">mission</span> </h1><br>
...
<?php 
        ...
        //  mysqli_select_db($con,'patients');
          ...
          $query="SELECT * FROM `hospitalvisittag` WHERE patientId = '$patientId' ORDER BY `hospitalvisittag`.`dateOfVisit` DESC" ;

          $queryExecute=mysqli_query($con,$query);

           // $total= mysqli_num_rows($queryExecute);

while($addmission =mysqli_fetch_array($queryExecute))
          {
  
?>
           <tr class="text-center">
              <td><?php echo $addmission['id'] ?></td>
              <td><?php echo $addmission['hospitalId'] ?></td>
              <td><?php echo $addmission['hospitalName'] ?></td>
              <td><?php echo $addmission['hospitalContact'] ?></td>
              <td><?php echo $addmission['patientId']?></td>
              <td><?php echo $addmission['patientName']?></td>
              <td><?php echo $addmission['dateOfVisit']?></td>
              <td><?php if($addmission['dateOfdischarge']==0) 
                  {
                     echo "<label class='text-warning'>"."Not Discharged Yet"."</label>";
                  }
                 else
                  {
                    echo"<label class='text-success'>".$addmission['dateOfdischarge']."</label>";
                  }


                 ?> 
              </td>
              <td><?php echo $addmission['suffering']?></td>
              <td><?php echo $addmission['remarks']?></td>
              <td><?php echo $addmission['doctorId']?></td>
              <td><?php echo $addmission['doctorName']?></td>          
              <td><?php echo ($addmission['status']==1) ? '<label class="text-warning">Active</label>' : '<label class="text-success">Discharged</label>'; ?></td>
      <?php 
    }
       ?>       
